...
public class ObjectPooler : MonoBehaviour
{
    ...
    void  Awake()
    {
        SharedInstance = this; // only  one instance of this per Scene
    ...
    // return GameObject reference of one of the requested objects in the pool when available
    public  GameObject  GetPooledObject(ObjectType type)
    {
        ...
        foreach (ObjectPoolItem item in itemsToPool)
        {
            if (item.type == type)
            {
                // instantiate the requested object at runtime if expandPool == true
                if (item.expandPool)
                {
                    GameObject pickup = (GameObject)Instantiate(item.prefab);
                    pickup.SetActive(false);
